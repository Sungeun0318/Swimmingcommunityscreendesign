...
import { TrendingUp, Flame, ChevronDown, Check, ChevronLeft, ChevronRight, X, ExternalLink, Crown } from 'lucide-react';

interface CommunityFeedProps {
  onCreatePost: () => void;
}

export function CommunityFeed({ onCreatePost }: CommunityFeedProps) {
  const [selectedFilters, setSelectedFilters] = useState<string[]>(['Following']);
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [currentSlide, setCurrentSlide] = useState(0);
  const [isPaused, setIsPaused] = useState(false);
  const [showTopAd, setShowTopAd] = useState(true);
  const [isPremium, setIsPremium] = useState(false); // 프리미엄 사용자 여부
  const carouselRef = useRef<HTMLDivElement>(null);

  const filterOptions = [
    { id: 'My', label: 'My Posts', color: 'from-purple-500 to-pink-500' },
  ...
  const toggleFilter = (filterId: string) => {
    setSelectedFilters(prev => {
      if (prev.includes(filterId)) {
        ...
        return newFilters.length === 0 ? ['Following'] : newFilters;
      } else {
  ...
  const filteredPosts = mockPosts.filter(post => {
    ...
    if (selectedFilters.includes('Following')) return true;
    if (selectedFilters.includes('My')) return false; // My posts would be filtered here
      ...
      <div className="space-y-3">
        {filteredPosts.map((post, index) => (
          <React.Fragment key={post.id}>
            ...
            {!isPremium && (index + 1) % 2 === 0 && index < filteredPosts.length - 1 && (
